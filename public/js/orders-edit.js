...
// Itens que já estavam no pedido mas cujo produto não existe mais no catálogo
// ativo entram como "produto legado": dá pra remover, mas não pra aumentar a
// quantidade além do que já estava (não temos mais estoque de referência).
(window.__ORDER_ITEMS || []).forEach(function (item) {
  if (!pdvProductsById[item.product_id]) {
    pdvProductsById[item.product_id] = {
      id: item.product_id,
      name: item.name + ' (removido do catálogo)',
...
let pdvCart = (window.__ORDER_ITEMS || []).map(function (item) {
...
function pdvRenderCart() {
  ...
  if (!pdvCart.length) {
    container.innerHTML = '<div class="hint" id="pdv-cart-empty">Nenhum item.</div>';
  } else {
...
async function pdvSalvarEdicao() {
  if (!pdvCart.length) {
    pdvShowMessage('A venda precisa ter ao menos um item — use "Cancelar venda" na lista de pedidos se quiser anular.', 'error');
    return;
  }
  ...
  try {
    const res = await fetch(`/pedidos/${window.__ORDER_ID}/editar`, {
      method: 'POST',
    ...
    pdvShowMessage('Venda atualizada!', 'success');
    setTimeout(() => { window.location.href = '/pedidos'; }, 800);
  } catch (err) {
